...
#    '''
    ...
    spectrum_singularity=spectrum<np.mean(spectrum)*1e-6
    if np.sum(spectrum_singularity)>0:
        indices=np.where(spectrum_singularity)[0]
        for i in range(indices.size):
            if indices[i]==0:
                spectrum[indices[i]]=spectrum[indices[i]+1]
            elif indices[i]==spectrum.size-1:
                spectrum[indices[i]]=spectrum[indices[i]-1]
            else:
                spectrum[indices[i]]=np.min([spectrum[indices[i]-1],spectrum[indices[i]+1]])
    # calculate the apodization vector based on the apodization mode
    if np.size(apodization)==np.size(spectrum):
    ...
    Ascan_z=np.fft.ifft(int0meanphase)
    
    return Ascan_z

def calc_Bscan(rawdata,spectrum,dechirp=0,Ascanav=1,apodization='hanning',popt=0,objective='LK4',filters='none'):
    '''
    ...
    if np.size(dechirp)==1:
        dechirp=np.fromfile('modules\\Chirp.data',np.float32)
    # interpolate to a linear grid in k space
    f = interp1d(dechirp, interference_signal, kind='cubic')
    ...
    if Ascanav>1:
        ...
        Bscan2=np.empty([N,int(rawdata.shape[1])],dtype=np.complex64)
        for i in range(N):
